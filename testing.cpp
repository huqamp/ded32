...
void unit_test_roots(void) {
    ...
    for (int i = 0; i < all; i++) {
        ...
        else if ((array[i].number_roots != InfinityRoot && array[i].number_roots != NoRoot) && (!(compare_roots(array[i].first_root, x1, 1E-2)) || !(compare_roots(array[i].second_root, x2, 1E-2)))) {
